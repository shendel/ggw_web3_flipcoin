...
interface IRandomGenerator {
    function requestRandom(bytes32 hash) external returns (uint256);
...
contract FlipCoinGame is ReentrancyGuard {
    // ==== Типы ====
    enum GameStatus { Pending, Won, Lost, Refund }
    ...
    function play(uint256 betAmount, bool chosenSide, bytes32 hash) external nonReentrant {
        ...
        uint256 requestId = randomGenerator.requestRandom(hash);
    ...
    function refund (uint256 gameId) external nonReentrant {
        ...
        require(game.gameId == gameId, "Game not founded");
        require(game.timestamp + refundTimeout <= block.timestamp, "Not ready for refund");
        PlayerAccount storage player = players[game.player];
        require((player.playerAddress == msg.sender) || (msg.sender == owner), "Only owner or player");
        depositManager.gameDepositUserDeposit(player.playerAddress, game.amount);
        game.status = GameStatus.Refund;
    }
    ...
    function fulfillRandom(uint256 requestId, bytes32 random) external {
        ...
        require(keccak256(abi.encodePacked(random)) == game.hash, "Invalid random for this hash");
        ...
        if (won) {
            ...
            depositManager.gameBurnAndStake(game.amount);
